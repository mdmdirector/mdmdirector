...
func main() {
	...
	flag.IntVar(&OnceIn, "once-in", env.Int("once-in", 60), "Number of minutes to wait before queuing an additional command for any device which already has commands queued. Defaults to 60. Ignored and overidden as 2 (minutes) if --debug is passed.")
	flag.Parse()
